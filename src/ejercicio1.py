...
def direccionCliente(clientes:list) ->str:
    '''
    Coge la última posición de la tupla, que es la dirección de los clientes.
    
    parameters
    ----------
    clientes(list): lista de los datos de los clientes
    
    returns
    -------
    Devuelve la dirección de los clientes.
    '''
    direcciones = set()
    for direccion in range(len(clientes)):
        direcciones.add((clientes[direccion][-1]))
    return direcciones
...
def main():
    ...
    #proceso y salida
    print(direccionCliente(clientes))
